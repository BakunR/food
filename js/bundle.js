        ...
        render() {
            const element = document.createElement('div');
                ...
                if (this.classes.length === 0){
                    ...
                    this.classes.forEach(className => element.classList.add(className));//робимо перебір класів які додались,хз скільки, після перебору вони додаються в element де лежить наш ДІВ
                }
...
function forms (formSelector, modalTimerId) {
    ...
    function bindPostData(form) {
        form.addEventListener('submit', (e) => {
            ...
            form.insertAdjacentElement('afterend',statusMessage);//вставляємо спінер пісоя форми
        
            const formData = new FormData(form);

            const json = JSON.stringify(Object.fromEntries(formData.entries()));// отримане  обєкт переводимо в  масиви, масиви переводимо в обєкт і переводимо в json формат
        ...
        function showThanksModal(message) {//текст після відправки  форми
            const prevModalDialog = document.querySelector('.modal__dialog');

            prevModalDialog.classList.add('hide'); // ховаємо контент в вікні
            (0,_modal__WEBPACK_IMPORTED_MODULE_0__.openModal)('.modal', modalTimerId);//відкриваємо вікно
            
            const thanksModal = document.createElement('div');// створюємо аналог аде вже з тексто який нам потрібен
...
window.addEventListener('DOMContentLoaded',() =>{
    ...
    (0,_modules_timer__WEBPACK_IMPORTED_MODULE_2__.default) ('.timer', '2021-07-25');
